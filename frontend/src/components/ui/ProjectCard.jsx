...
const ProjectCard = () => {
  ...
  return (
    <section id="projects" className="py-8 max-w-6xl mx-auto px-4 sm:py-10 md:py-12 animate-fade-in overflow-x-hidden">
     <WhatDone/>
      <h1 className="text-4xl font-bold my-4 text-center text-white">My Projects</h1>
        ...
        {projects.map((project, index) => {
          ...
          return (
            <div
              ...
              className={`bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-xl shadow-lg transform transition my-4 duration-300 hover:scale-105 hover:shadow-xl relative ${animationClass} ${
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {project.tags.map((tag, i) => (
                  <span
                    key={i}
                    className="text-xs bg-white/10 backdrop-blur-md border border-white/20 text-amber-500 px-2 py-1 rounded-full"
                  >
                    #{tag}
                  </span>
                ))}
